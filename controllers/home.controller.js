...
// Used in alert emails & share links
// =======================
exports.legacyPropertyRedirect = async (req, res) => {
  try {
    ...
    // Render home page and let EJS auto-open the Bootstrap modal
    res.render('pages/home', {
      email: req.session.adminEmail || null,
      property,
...
exports.enquiryHandler = async (req, res) => {
  try {
    ...
    await resend.emails.send({
      from: 'GS Infra Estates <alerts@gsinfraestates.com>',
      to: process.env.ADMIN_EMAIL || 'your-admin@example.com',
      subject: 'New Property Enquiry - GS Infra Estates',
      html,
    });

    // Optional: handle subscribe flag here (e.g., create AlertSubscriber)

    res.redirect('/?enquiry=success');
  } catch (err) {
    console.error('❌ Enquiry handler error:', err);
    res.redirect('/?enquiry=error');
  }
};
